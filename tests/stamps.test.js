...
test('takeMonthlyRecap: 月が変わって最初の1回だけ、先月の個数をねぎらう', () => {
  const s = fakeStorage();
  stampToday(s, new Date(2026, 5, 3)); // 6月に3回
  stampToday(s, new Date(2026, 5, 10));
  stampToday(s, new Date(2026, 5, 24));
  const first = takeMonthlyRecap(s, new Date(2026, 6, 1));
  assert.ok(first.includes('6月') && first.includes('3個'), first);
  assert.ok(!/がんばりが足りない|少ない/.test(first)); // 責めない
  // 2回目は出さない
  assert.equal(takeMonthlyRecap(s, new Date(2026, 6, 2)), null);
});

test('takeMonthlyRecap: 先月スタンプなし・年またぎも正しく', () => {
  const s = fakeStorage();
  assert.equal(takeMonthlyRecap(s, new Date(2026, 6, 1)), null); // 先月ゼロなら何も言わない
  const s2 = fakeStorage();
  stampToday(s2, new Date(2025, 11, 31)); // 12月に1個
  const msg = takeMonthlyRecap(s2, new Date(2026, 0, 5));
  assert.ok(msg.includes('12月') && msg.includes('1個'), msg);
});

test('milestoneMessage: 月末付近（28・30・31個）にもご褒美がある', () => {
  assert.ok(milestoneMessage(28));
  assert.ok(milestoneMessage(30));
  assert.ok(milestoneMessage(31));
});
